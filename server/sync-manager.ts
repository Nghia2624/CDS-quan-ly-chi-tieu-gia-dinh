...
export class SyncManager {
  ...
  async getSyncStatus(familyId: string): Promise<{
    ...
    const hasChanges = recentExpenses.some(expense => 
      new Date(expense.createdAt) > lastSync || 
      (expense.updatedAt && new Date(expense.updatedAt) > lastSync)
  ...
  async checkConflicts(familyId: string, clientData: any[]): Promise<{
    ...
    for (const item of clientData) {
      if (item.id && item.updatedAt) {
        ...
        if (serverItem.length > 0 && serverItem[0].updatedAt > item.updatedAt) {
          conflicts.push({
            type: 'expense',
            id: item.id,
            clientVersion: item,
  ...
  async getSyncStats(familyId: string): Promise<{
    ...
    const expenses = await db.select().from(expenses).where(eq(expenses.familyId, familyId));
    const users = await db.select().from(users).where(eq(users.familyId, familyId));
    ...
    const dataSize = JSON.stringify({ expenses, users }).length;
