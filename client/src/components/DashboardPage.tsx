...
export function DashboardPage() {
  ...
  const getCurrentMonthExpenses = () => {
    // Lấy expenses của THÁNG HIỆN TẠI
  ...
  const getExpenseTrend = () => {
    ...
    // Tháng hiện tại: từ đầu tháng đến hôm nay
    ...
    // Tháng trước: tính tỷ lệ tương ứng (để so sánh công bằng)
    // Nếu tháng này đã qua X ngày, so sánh với X ngày đầu tháng trước
    const lastMonthExpensesForComparison = expensesData.expenses
      .filter((exp: any) => {
        if (!exp.createdAt) return false;
        const expDate = new Date(exp.createdAt);
        // So sánh với cùng số ngày trong tháng trước
    ...
    // Nếu không có dữ liệu tháng trước, return 0
    if (lastMonthExpensesForComparison === 0) {
      // Nếu tháng này có chi tiêu nhưng tháng trước không có, coi như tăng 100%
                    ...
                    <div key={member.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {memberExpenses.length} giao dịch
                        </p>
                      </div>
                      <Badge variant="secondary">
